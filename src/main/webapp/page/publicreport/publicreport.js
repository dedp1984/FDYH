	...
	launch : function() {
			...
			buttons:[{
				...
				handler:function(){
					if(queryForm.getForm().isValid()){
						...
						gridStore.setProxy({
							type: 'ajax',
				            url: '../../action/query/queryPublicBusinessData',
				            reader: {  
			                    type:'json'
							}
						});
		...
		var gridStore = Ext.create('Ext.data.TreeStore', {
	        ...
	        fields: [
	 	            ...
	 	            {name: 'depositbal',     type: 'string'},
	 	            {name: 'depositavg', type: 'string'},
	 	            {name: 'depositbaldate', type: 'string'},
	 	            {name: 'depositavgdate', type: 'string'},
	 	            {name: 'creditbal', type: 'string'},
	 	            {name: 'creditdate', type: 'string'},
	 	            {name: 'creditavg', type: 'string'}
	 	        ]
	    });
				...
				items:['->',{
					...
					handler:function(){
						...
						var requestUrl=Ext.String.format(
					            '../../action/export/ExportPublicBusinessData?enddate={0}&branchid={1}&accountid={2}',
