				...
				items:['->',{
					...
					handler:function(){
						if(editForm.getForm().isValid()){
							...
							var accounttype=editForm.getForm().findField('accounttype').getValue();
							if(accounttype=='4'||accounttype=='5'||accounttype=='7'||accounttype=='8'){
								if(totalPercent<100){
									Ext.Msg.alert('信息提示','个人类型账户分配比例小于100,请重新分配');
									return;
								}
							}
			...
			columns : [ {
				...
				items : [ {
					...
					handler : function(grid, rowIndex, colIndex, item, e) {
						...
						editForm.getForm().findField('branchid').setValue(record.get('branchid'));	
						editForm.getForm().findField('branchname').setValue(record.get('branch.branchname'));	
						if(record.get('accounttype')=='4'||record.get('accounttype')=='5'||record.get('accounttype')=='7'||record.get('accounttype')=='8'){
							Ext.getCmp('save').setText('确认分配关系');
						}else{
							Ext.getCmp('save').setText('提交分配申请');
						}
						queryPanel.hide();
