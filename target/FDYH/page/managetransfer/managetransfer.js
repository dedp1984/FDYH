Ext.application({
	...
	launch : function() {
		...
		var changeBranchForm=Ext.create('Ext.form.Panel',{
			bodyPadding:'10 10 0',
			anchor:'100% 15%',
			title:'选择变更目的部门',
			layout:'form',
						...
						readOnly:true
							...
							handler:function(button,e){
								...
								var treePanel = Ext.create('Ext.tree.Panel', {
									...
									listeners:{
										itemclick:function(view,record,item,index,e,opt){
											changeBranchForm.getForm().findField('changebranchid').setValue(record.get('id'));
											changeBranchForm.getForm().findField('changebranchname').setValue(record.get('text'));
											win.close();
										}
									}
								});
		...
		var editForm =Ext.create('Ext.form.Panel',{
			anchor:'100% 100%',
			layout:'anchor',
			items:[changeBranchForm,ownAccountGrid,transAcccountGrid],
			dockedItems : [{
				xtype:'toolbar',
				dock:'top',
				items:['->',{
					...
					handler:function(){
						...
						var changeBranchId=changeBranchForm.getForm().findField('changebranchid').getValue();
						var paramstr='';
